...
set "MARKER=%~dp0latest_build_dir.txt"
if not exist "%MARKER%" (
    echo Build marker not found: %MARKER%
    echo Please build with build_tsf_in_venv2.py first.
    pause
    exit /b 1
)

set /p BUILD_DIR=<"%MARKER%"
set "DLL=%BUILD_DIR%\bin\WubiIME_TSF.dll"
if not exist "%DLL%" (
...
echo Registering TSF IME from %BUILD_DIR%...
regsvr32 /s "%DLL%"
if %errorlevel% neq 0 (
